...
TEST_GROUP(Test_UnDirected_Char) { };
...
TEST(Test_UnDirected_Char, create) {
    GRAPH_CREATE(g)
    g.addVertex('a');
    CHECK_TRUE(g.getVertex('a') == 'a');
}

TEST(Test_UnDirected_Char, link) {
    GRAPH_CREATE(g)
    g.addVertex('1');
    g.addVertex('2');
    g.link('1', '2');
...
TEST(Test_UnDirected_Char, equals) {
...
TEST(Test_UnDirected_Char, cycle) {
...
TEST(Test_UnDirected_Char, no_cycle) {
...
TEST(Test_UnDirected_Char, NoDuplicates) {
    GRAPH_CREATE(g)
    g.addVertex('1');
    CHECK_TRUE(g.getVertex('1')=='1');
}

TEST(Test_UnDirected_Char, graphCopy) {
    GRAPH_CREATE(g)
    g.addVertex('1');
    g.addVertex('2');
    g.addVertex('3');
    ...
    CHECK_TRUE(g.getVertex('3')==h.getVertex('3'));
    CHECK_TRUE(g.isLinked('2', '1')==h.isLinked('2', '1'));
}
